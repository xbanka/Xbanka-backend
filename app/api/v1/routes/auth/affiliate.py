...
from app.core.email import send_forgot_password_email, send_verification_email
...
def refresh_token(request: Request, response: Response):
    ...
    access_token, refresh_token = AuthService.refresh_access_token(
        current_refresh_token, "affiliate"
    )
...
def verify_magic_link(token: str, db: Session = Depends(get_db)):
    ...
    affiliate: Affiliate = AuthService.verify_magic_link(
        db, Affiliate, token, credentials_exception
    )
...
def reset_password(reset_request: ResetPasswordRequest, db: Session = Depends(get_db)):
    ...
    AuthService.update_user_password(
        db, user_token, reset_request.new_password, AffiliateService
    )
