...
class AffiliateService(Service):
    ...
    def create(db: Session, obj_in: RegisterBase) -> Affiliate:
        ...
        stmt = select(Affiliate).where(
            or_(Affiliate.email == obj_in.email, Affiliate.username == obj_in.username)
        )
        ...
        if affiliate:
            raise HTTPException(
                status_code=400, detail="Affiliate with email/username already exists"
            )
    ...
    def get_by_refcode(db: Session, refcode: str) -> Affiliate:
        ...
        if not affiliate:
            raise HTTPException(
                status_code=404, detail="Affiliate with this referral code not found"
            )
        return affiliate
    ...
    def set_codename(db: Session, affiliate: Affiliate, codename: str) -> Affiliate:
        ...
        if existing:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Codename already taken"
            )
    ...
    def record_unique_visit(
        db: Session, affiliate: Affiliate, request: Request, response: Response
    ) -> None:
    ...
    def get_payouts(
        ...
        total = (
            db.scalar(
                select(func.count()).select_from(
                    select(Payout).where(Payout.affiliate_id == affiliate_id).subquery()
                )
            )
            or 0
        )
    ...
    @staticmethod
    def export_payouts(
        db: Session, affiliate_id, status: Optional[PayoutStatusEnum] = None
    ):
        stmt = select(Payout)
        ...
        stmt = stmt.where(Payout.affiliate_id == affiliate_id).order_by(
        ...
        ws.append(["Amount", "Status", "Payment Reference", "Paid At"])  # header
        ...
        for payout in payouts:
            ...
            ws.append([payout.amount, payout.status.value, payout.payment_ref, paid_at])
    ...
    def update_bank_details(
        db: Session, affiliate: Affiliate, bank_details: UpdateBankDetailsRequest
    ) -> None:

        # Validate Nigerian bank account number (10 digits)
        if not is_valid_account_number(bank_details.account_number):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid account number"
            )
